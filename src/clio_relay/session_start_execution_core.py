"""Owned remote relay session start execution -- the exact start path.

split/session-start-execution-w3 (#231): ``execute_owned_session_start``
moved out of session_start_execution.py, which stays the resident facade
re-exporting it (and the identity-challenge/promotion helpers that moved to
session_start_identity_challenge.py / session_start_promotion.py alongside
it) under their original names.

``execute_owned_session_start`` alone is ~910 lines of crash-recovery start
logic (systemd containment, broker handoff, resumable-attempt promotion)
that does not decompose along a clean second seam without restructuring the
function itself -- the same call the file's own prior RATCHET_BASELINE
entry in scripts/check_file_size.py already made (split/session-lifecycle
slice J, #231) and that session_recovery_inspection.py's docstring cites as
precedent for its own sibling split. Almost every local computed here
(``existing``, ``prior_attempt``, ``resumable_attempt``,
``recorded_generation``, ``attempt_identity``, ``selected_generation``,
``owner_token``, ``systemd_unit``/``systemd_description``,
``containment_identity``) threads through the entire body across several
early-return branches and two closures
(``persist_containment``/``release_child_environment``) that read and
mutate the enclosing scope; splitting it into standalone top-level
functions would mean converting that threading into an explicit
multi-parameter API -- a semantic rewrite of security-sensitive
crash-recovery code, out of scope for a mechanical, behavior-preserving
extraction. It moves here as one atomic, unsplit unit instead, matching the
cli_session_teardown.py / cli_owned_report_artifact.py precedent for the
same shape of problem.

session_lifecycle is imported back INSIDE the function (not at module
scope): session_lifecycle imports session_start_execution.py back for its
cli.py-compatibility re-export block, so a module-scope back-import here
would create a load-order-dependent circular import -- deferred to call
time, it is import-order-independent, matching the pattern the original
module established.
"""
...
from clio_relay.session_start_promotion import _promote_resumable_contained_start
