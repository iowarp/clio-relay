...
# repointing its bare imports for the five names below to their real new
# homes (session_lifecycle_report.py, session_start_query.py,
# session_start_wait.py) is a net LOC increase there (four new
# single-purpose `from module import name` statements cannot collapse
# into fewer lines than the names removed from the old block) and would
# regress that ratchet. cli.py's own imports and every call site stay
# byte-for-byte unchanged; only session_lifecycle.py re-exports these
# under their original names so `from clio_relay.session_lifecycle import
# X` keeps resolving. Every OTHER consumer (this module's own internal
# calls, tests, transport_probe.py) is repointed to the real owner
# module -- this is the one deliberate exception, not a default escape
# hatch, and should be deleted the moment cli.py's own split lands and can
# absorb the "one-line import repoint" itself.
