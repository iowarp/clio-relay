...
# live_acceptance_wait.py (#231 rework) owns these -- bounded remote
# observation waits, re-exported the same qualified way for the
# still-resident facade's call sites and test_live_acceptance.py's direct
# imports. _wait_for_success IS a monkeypatch target (test_live_acceptance
# .py) with three still-resident callers in _run_live_acceptance below, so
# its call sites are qualified rather than routed through a bare
# re-export -- a bare reference would not observe a patch applied to the
# new module after import (same fix as the browser-evidence and
# job-verification slices).
...
# live_acceptance_checkpoint.py (#231 rework) owns these -- resumable
# checkpoint intent, resume validation, and pending-observation recording,
# re-exported the same qualified way for the facade below. None are
# monkeypatched or imported directly by tests.
_current_live_acceptance_intent = (
    live_acceptance_checkpoint._current_live_acceptance_intent  # pyright: ignore[reportPrivateUsage]  # noqa: SLF001
)
