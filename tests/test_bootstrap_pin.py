...
def test_a_dead_generation_pin_is_repointed_at_the_produced_runtime(tmp_path: Path) -> None:
    ...
    record = reconcile_cluster_runtime_pin(cluster="ares", registry_path=path)
...
def test_reconciliation_is_idempotent(tmp_path: Path) -> None:
    ...
    reconcile_cluster_runtime_pin(cluster="ares", registry_path=path)
    second = reconcile_cluster_runtime_pin(cluster="ares", registry_path=path)
