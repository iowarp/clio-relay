...
| **R11** | [`core_queue.py` split design](core-queue-split-2026-08.md): AST-derived disjoint 16,137-line ledger, call-site DAG, pinned patch audit, merged-overlap analysis, and implementation slices | **DESIGN COMPLETE** — adversarial findings incorporated; implementation not started | this docs-only revision | `feat/231-r11-core-queue-design` | No source or ratchet change; the owner-transfer plus deletion ledger closes exactly, migration follows the documented call-site method, every planned owner cap is at most 750 lines, and each implementation slice must ratchet `core_queue.py` down. |
