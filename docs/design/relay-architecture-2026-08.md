...
| **R8(iii)+** | `session_lifecycle.py` wire-model extraction (§4.4) | PLANNED — unchanged. The v1.7.0 campaign added `session_attach.py` (clio-relay#276 B1) as a NEW sibling owner (attach/last-session-record semantics), which relieves pressure on `session_lifecycle.py` but is not the §4.4 extraction itself | — | — | — |
| **R11** | [`core_queue.py` split design](core-queue-split-2026-08.md): AST-derived disjoint 16,137-line ledger, call-site DAG, pinned patch audit, merged-overlap analysis, and implementation slices | **PARTIALLY IMPLEMENTED** — the 1.6.7 release landed the first wave (8 queue consumer/diagnosis modules: admission simulation/snapshot, diagnosis(+constants), listing, stale recovery, worker capacity/status) plus `queue_owner_session_lease` (#277) and `queue_progress`'s bounded window read (#214); the split-architecture guard caught up with all of it in the v1.7.0 campaign (`93da20f` — owner ranks/budgets, the 134→135 facade ledger, and the NON-owner registration for the 8 consumer modules, which take a live `ClioCoreQueue` parameter and are deliberately not owners) | `3b759cc` (1.6.7 wave), `6306fb6` (#277), `93da20f` (guard catch-up) | develop | Each landed wave ratcheted `core_queue.py` down per the design's rule. |

**v1.7.0 campaign note (2026-08-26).** The release campaign (plan: issues
#275-#285) continued the owner-module discipline outside the R-numbered
decomposition slices. New owner modules landed, all sweet-spot-sized:
`session_attach.py` (#276), `worker lifetime lock sidecar` additions (#202),
`http_api_streaming` console-SSE lane + `remote_connection_stream_io`
additions (#221/#259), `application_runtime_prediction.py` +
`execution_watch_prediction.py` (#214), `frpc_unit.py` proxy bring-up (#279),
`queue_owner_session_lease.py` (#277), `frp_visitor_reconciliation.py` +
`frp_visitor_process_inspection.py` (#285), `remote_connection_lease.py`,
`release_pin_validation.py`. The file-size gate additionally enforces a
ratcheting DISTRIBUTION floor (#280): percent of files under 500 lines may
only rise — src floor 77.36%, tests floor 63.15% at this writing (target
90%). Named next-split debts recorded in merge messages: `execution_watch.py`
(798/800) and `tests/test_frp_transport_dials.py` (1286) must split on next
touch.
...
| Release-identity pin sites that must move together on a version bump (#198) | **R7 DONE.** 80 sites (43 line, 25 regex, 7 filename, 2 key, 2 derived-digest, 1 placeholder — final count as of §7.9's post-review pass, incl. the R6-fix A6 addendum), was 11+ (measured at 1.6.5; §7's recount found the true count much higher once fixture/mirror sites were included) | 1 (`release_pins.py` + `release_pin_sites.py`'s `PinSite` table) + 1 bump command (`scripts/bump_release_version.py`) + 1 preflight (`scripts/check_release_identity.py`, wired into `local.release-identity`) |
...
**Final site count per selector kind** (80 total; 78 mutable/
agreement-checked, 2 frozen/tracked-only, 4 `dynamic_path`):

| Kind | Count | What it covers |
|---|---|---|
| `line` | 43 | Stable, well-known source lines and fixture lines — the canonical definitions, their mirrors, and (post-review) the release-gate description prose and the five `docs/*.md`/`docs/ai/*.md` staging-gate prose sites B3 found stale. Includes two `tests/test_endpoint.py` `expected_registered_contract` sites the completeness sweep named as a real drift-in-progress during the R6-fix review pass (A6): a genuinely new `v3.7` literal the sweep is designed to catch, not a stale test assertion left un-registered. |
...
By family: `relay_version` 4, `matrix_digest` 2, `jarvis_contract` 40,
`kit_version` 34.
