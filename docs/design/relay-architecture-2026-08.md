...
| **R11** | [`core_queue.py` split design](core-queue-split-2026-08.md): line-backed concern inventory, owner-module map, patch seams, migration order, overlap/deletion analysis, exit criteria, and implementation issue slices | **DESIGN COMPLETE** — implementation not started | this docs-only commit | `feat/231-r11-core-queue-design` | No source or ratchet change; every planned owner module is capped below 800 lines and each implementation slice must ratchet `core_queue.py` down. |
